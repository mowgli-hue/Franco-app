...
// ─── LIVE VIDEO CALL WITH SOPHIE (HeyGen LiveAvatar, FULL mode) ───────────────
// Photorealistic real-time avatar. HeyGen retired the old "Interactive Avatar"
// streaming API on 2026-03-31; this now uses **LiveAvatar** (@heygen/liveavatar-
// web-sdk). FULL mode: HeyGen runs the listening + LLM + speech, so it holds the
// whole conversation. Sophie's personality lives in a LiveAvatar "Knowledge Base"
// (context), and the avatar + ids are configured SERVER-SIDE in api/heygen-token.js.
//
// TO TURN THIS ON (then it ships OTA via Capgo):
//   1. npm i @heygen/liveavatar-web-sdk
//   2. In LiveAvatar dashboard: pick/create an avatar + a Knowledge Base (context),
//      add account balance. Note the avatar_id + context_id (+ optional voice_id).
//   3. In Vercel set: HEYGEN_API_KEY, LIVEAVATAR_AVATAR_ID, LIVEAVATAR_CONTEXT_ID,
//      LIVEAVATAR_LANGUAGE (fr), and LIVEAVATAR_SANDBOX=false when ready to bill. Redeploy.
//   4. Set HEYGEN_ENABLED = true.
...
function LessonVideoCall({ lesson, learner, onClose }){
  ...
  const hangUp = ()=>{ try{ avatarRef.current?.stop?.(); }catch{} onClose(); };
...
function SophieLive({ onClose }){
  ...
  const sysPrompt = useMemo(()=> buildSophieSystemPrompt({ learner:{ name:learnerName }, lesson:null })
    + `\n\nLIVE TUTORING MODE: You are Sophie in a real-time spoken tutoring session, like a human teacher on a video call. You can teach and practise EVERY skill the learner asks for: speaking & pronunciation, listening, reading, writing, grammar, vocabulary, real-life Canadian conversation (clinic, landlord, job, bank, transit, pharmacy), and CLB / TEF Canada / TCF exam strategy. Read what they need and lead a short activity: ask one thing at a time, wait for their answer, correct gently, praise specifically, then continue. Keep every reply short and natural to say aloud (2-3 sentences), mixing simple French with English. Never lecture — converse, like a warm real teacher.`
  , [learnerName]);
  ...
  return <div style={{position:"fixed",inset:0,zIndex:400,background:"linear-gradient(160deg,#0B1020,#1E1B4B)",display:"flex",flexDirection:"column"}}>
    ...
    {!started
      ...
      : <><div style={{padding:"0 12px 8px",display:"flex",gap:7,flexWrap:"wrap",justifyContent:"center"}}>
          {[
            {l:"🗣️ Speaking", p:"Let's practise speaking. Give me one short French sentence to say, then check my pronunciation."},
            {l:"🎧 Listening", p:"Let's practise listening. Say one short French sentence slowly and ask me what it means."},
            {l:"📖 Reading", p:"Give me a short French sentence to read and ask me one question about it."},
            {l:"✍️ Writing", p:"Give me a short writing prompt in French for daily life in Canada, then correct my answer."},
            {l:"🔤 Grammar", p:"Teach me one useful French grammar point simply, with an example, then quiz me once."},
            {l:"📝 Exam (CLB/TEF)", p:"Ask me one TEF Canada style speaking question and coach my answer."},
          ].map(c=>(
            <button key={c.l} onClick={()=>ask(c.p)} disabled={status==="thinking"}
              style={{background:"rgba(255,255,255,0.12)",color:"#fff",border:"none",borderRadius:50,padding:"7px 12px",fontSize:12,fontWeight:700,cursor:status==="thinking"?"default":"pointer"}}>{c.l}</button>
          ))}
        </div>
        <div style={{padding:"0 14px calc(env(safe-area-inset-bottom) + 14px)",display:"flex",gap:8,alignItems:"center"}}>
